...
export default class PartySubscriptionsModel extends BaseModel {
  ...
  isFree = (partyId, tableNumber) => {
    return this.querySingle(
      `SELECT * FROM ${this.table} WHERE party_id = ? and table_number = ? AND status != "rejected"`,
      [partyId, tableNumber]
    )
  ...
  partyReservedTables = partyId => {
    return this.querySingle(
      `SELECT GROUP_CONCAT(table_number) as reserved_tables FROM ${this.table} WHERE party_id = ? AND status != 'rejected'`,
      [partyId]
    )
